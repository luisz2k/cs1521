...
# registers $t3 and $t4 used to hold temporary results PLUS $t6
# $t5 = numbers[i]
...
while: 
    ...
    beq $t1, $t2, while_end     # while (i < n_seen) {
    ...
    beq $t0, $t5, while_end    # if (x == numbers[i]) break;
    addi $t1, $t1, 1    # i++;

    b while
while_end:
    beq $t1, $t2, n_seen    # if (i == n_seen) {
