...
static void execute_command(char **words, char **path, char **environment) {
    ...
    // implementing cd 
    // if an argument is entered after cd, cd to the argument name 
    ...
    if (program[0] == '.' || program[0] == '/') {     // calling non built-in function 
        if (is_executable(program)) {
            posix_spawn(&pid, words[0], NULL, NULL, words, environment);
    ...
    } else {    // calling built-in function
        char fullProg[1000];
        int i = 0;
        while(path[i] != NULL) {
            strcpy(fullProg, path[i]);
            strcat(fullProg, "/");
            strcat(fullProg, program);
            if (is_executable(fullProg)) {
                posix_spawn(&pid, fullProg, NULL, NULL, words, environment);
                if (waitpid(pid, &exit_status, 0) == -1) {
                    perror("waitpid");
                    return;
                }
                printf("%s exit status = %d\n", fullProg, WEXITSTATUS(exit_status));
                return;
            }
            i++;
        }
