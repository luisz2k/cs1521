...
static void execute_command(char **words, char **path, char **environment) {
    ...
    pid_t pid;
    //extern char **environ;
    char *prog_argv[] = {words[0], words[1], NULL};

    //if user seems to know the location by passing ./program or /bin/program
    if(program[0] == '.' || program[0] == '/'){
        //check whether the program is executable with hthe function is_executable
        if (is_executable(program)) {
            //if it is. run it with posix_spawn
            posix_spawn(&pid, words[0], NULL, NULL, prog_argv, environment);
            return;
        } else {
        ...
        //user seems to want to directly use the built-in program
        //concatinate path[i]/program
        //pass that to is_executable
        //if it is. run it with posix_spawn
        //if not, print error message
        ...
        char fullProg[1000];
        strcpy(fullProg, path[1]);
        strcat(fullProg, "/");
        strcat(fullProg, program);
        printf("new path: %s\n", fullProg);

        if (is_executable(fullProg)) {
            posix_spawn(&pid, words[0], NULL, NULL, prog_argv, environment);
        }
